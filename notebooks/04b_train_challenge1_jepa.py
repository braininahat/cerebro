# %% [markdown]
# # Challenge 1: Supervised Response Time Prediction with SignalJEPA_PreLocal
#
# This notebook trains a SignalJEPA_PreLocal (transformer-based) model on the
# Contrast Change Detection (CCD) task to predict response time (RT) from
# stimulus-locked EEG windows.
#
# ## Pipeline Overview
...
# 5. Train SignalJEPA_PreLocal with PyTorch Lightning
# 6. Save weights for submission
#
# ## Key Differences from notebook 04 (EEGNeX)
# - **Architecture**: Transformer-based (SignalJEPA) vs CNN-based (EEGNeX)
# - **Hyperparameters**: More complex (feature encoder, positional encoding, transformer layers)
# - **Model size**: Larger parameter count
#
...
class Challenge1Module(L.LightningModule):
    ...
    def __init__(self, lr=1e-3, weight_decay=1e-5, epochs=100):
        ...
        # Model: SignalJEPA_PreLocal with transformer architecture
        self.model = SignalJEPA_PreLocal(
            n_chans=N_CHANS,
            n_outputs=N_OUTPUTS,
            n_times=int(WINDOW_LEN * SFREQ),  # 2 seconds at 100 Hz
            sfreq=SFREQ,
            # SignalJEPA-specific hyperparameters
            n_spat_filters=4,
...
wandb_logger = WandbLogger(
    ...
    name=f"challenge1_jepa_bs{BATCH_SIZE}_{timestamp}",
    tags=["challenge1", "supervised", "signaljepa", "baseline"],
    save_dir=RUN_DIR,
...
logger.info("="*60)
logger.info(f"Experiment: Challenge 1 Baseline (SignalJEPA_PreLocal)")
