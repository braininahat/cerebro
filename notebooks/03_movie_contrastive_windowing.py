...
dataset_movie = EEGChallengeDataset(
    task="DespicableMe", release="R5", cache_dir=DATA_ROOT, mini=USE_MINI
)
...
print(
    metadata[
        ["subject_id", "movie_id", "time_offset_seconds", "i_window_in_trial"]
    ].head(10)
)
...
for movie_name in movies:
    ...
    ds = EEGChallengeDataset(
        task=movie_name, release="R5", cache_dir=DATA_ROOT, mini=USE_MINI
    )
...
for movie_name in movies:
    ...
    print(
        f"  Total across all subjects: {stats['total_duration']:.1f}s ({stats['total_duration']/60:.1f} minutes)"
    )
...
perfect_alignment_groups = all_metadata.groupby(["movie_id", "time_offset_seconds"])[
    "subject_id"
].nunique()
...
print(
    f"  Percentage valid for positive pairs: {100*len(perfect_pos_pairs)/len(perfect_alignment_groups):.1f}%"
)
...
print(
    f"  Percentage valid for positive pairs: {100*len(binned_pos_pairs)/len(binned_groups):.1f}%"
)

print(f"\n⚠️  Impact of perfect alignment:")
print(
    f"  Positive pair groups: {len(perfect_pos_pairs)} vs {len(binned_pos_pairs)} (binned)"
)
...
projection_results = []
for config in stride_configs:
    r5_windows = window_count_df[window_count_df["config"] == config["name"]][
        "total_windows_r5"
    ].values[0]
    ...
    [
        {
            ...
            "Releases": 1,
            "Subjects/movie (avg)": len(all_metadata)
            // (len(movies) * (len(all_metadata) // len(movies) // 20)),
...
""".format(
        projected_perfect_pos_groups, projected_total_windows
    )
)
...
pair_availability = all_metadata.groupby(["movie_id", "time_bin"])[
    "subject_id"
].nunique()
...
print(
    f"Percentage valid for positive pairs: {100*len(valid_pos_pairs)/len(pair_availability):.1f}%"
)
