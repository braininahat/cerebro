...
print(
    f"Anchor: movie={anchor_meta['movie_id']}, "
    f"time_bin={anchor_meta['time_bin']}, "
    f"subject={anchor_meta['subject_id']}"
)
print(
    f"Positive: movie={pos_meta['movie_id']}, "
...
# Verify constraints
assert (
    anchor_meta["movie_id"] == pos_meta["movie_id"]
), "Positive should have same movie!"
assert (
    anchor_meta["time_bin"] == pos_meta["time_bin"]
), "Positive should have same time bin!"
assert (
    anchor_meta["subject_id"] != pos_meta["subject_id"]
), "Positive should have different subject!"
...
# Verify constraint
assert (
    anchor_meta["movie_id"] != neg_meta["movie_id"]
), "Negative should have different movie!"
...
print(
    f"  Same movie: {movies_pos_match}/{n_samples} ({100*movies_pos_match/n_samples:.0f}%)"
)
print(
    f"  Same time bin: {times_pos_match}/{n_samples} ({100*times_pos_match/n_samples:.0f}%)"
)
print(
    f"  Different subject: {subjects_pos_diff}/{n_samples} ({100*subjects_pos_diff/n_samples:.0f}%)"
)

print(f"\nNegative pairs ({n_samples} samples):")
print(
    f"  Different movie: {movies_neg_diff}/{n_samples} ({100*movies_neg_diff/n_samples:.0f}%)"
)
...
assert (
    subjects_pos_diff == n_samples
), "All positive pairs should have different subjects!"
...
for neg_strategy in [
    "diff_movie_mixed",
    "diff_movie_same_subj",
    "diff_movie_diff_subj",
]:
