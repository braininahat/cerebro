...
def validate(model, loader, device):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(loader, desc="Validation")):
            ...
            if batch_idx == 0:
                print(f"\n  🔍 Diagnostic: Checking where collapse happens...")
                print(f"     (Note: EEG in volts, typical range 10-100 µV = 1e-5 to 1e-4 V)")

                # Check INPUT diversity first (in microvolts for interpretability)
                input_std = X.std(dim=0).mean().item()
                input_range = (X.max() - X.min()).item()
                input_mean = X.mean().item()
                input_sample_diversity = X.std(dim=(1,2)).mean().item()

                print(f"\n     📊 Input X ACROSS samples:")
                print(f"        std = {input_std:.3e} V ({input_std*1e6:.2f} µV)")
                print(f"        range = {input_range:.3e} V ({input_range*1e6:.2f} µV)")
                print(f"        mean = {input_mean:.3e} V, CoV = {input_std/(abs(input_mean)+1e-10):.3f}")

                print(f"\n     📊 Input X WITHIN samples:")
                print(f"        std = {input_sample_diversity:.3e} V ({input_sample_diversity*1e6:.2f} µV)")

                # After spatial encoder (normalized, no µV conversion)
                spatial_out = model.spatial_encoder(X)  # (batch, n_times, d_model)
                spatial_std = spatial_out.std(dim=0).mean().item()
                spatial_range = (spatial_out.max() - spatial_out.min()).item()
                spatial_mean = spatial_out.mean().item()

                print(f"\n     📊 After spatial encoder:")
                print(f"        std = {spatial_std:.3e}, range = {spatial_range:.3e}")
                print(f"        mean = {spatial_mean:.3e}, CoV = {spatial_std/(abs(spatial_mean)+1e-10):.3f}")
                if spatial_std < 1e-8 and spatial_range < 1e-8:
                    print(f"        ⚠️  TRUE COLLAPSE: Both std and range near zero!")

                # After mamba blocks
                x = spatial_out
                for i, (mamba, norm) in enumerate(zip(model.mamba_blocks, model.layer_norms)):
                    residual = x
                    x = mamba(x)
                    x = norm(x + residual)
                mamba_out = x
                mamba_std = mamba_out.std(dim=0).mean().item()
                mamba_range = (mamba_out.max() - mamba_out.min()).item()
                mamba_mean = mamba_out.mean().item()

                print(f"\n     📊 After mamba blocks:")
                print(f"        std = {mamba_std:.3e}, range = {mamba_range:.3e}")
                print(f"        mean = {mamba_mean:.3e}, CoV = {mamba_std/(abs(mamba_mean)+1e-10):.3f}")
                if mamba_std < 1e-8 and mamba_range < 1e-8:
                    print(f"        ⚠️  TRUE COLLAPSE: Both std and range near zero!")

                # After pooling
                x = x.transpose(1, 2)  # (batch, d_model, n_times)
                pooled = model.pooler(x)  # (batch, d_model)
                pooled_std = pooled.std(dim=0).mean().item()
                pooled_range = (pooled.max() - pooled.min()).item()

                print(f"\n     📊 After pooling:")
                print(f"        std = {pooled_std:.3e}, range = {pooled_range:.3e}")

                # Check if pooled embeddings are identical across batch
                pooled_sample_std = pooled.std(dim=1).mean().item()  # Std across features per sample
                pooled_batch_std = pooled.std(dim=0).mean().item()   # Std across samples per feature

                print(f"\n     📊 Pooled embedding diversity:")
                print(f"        WITHIN samples (features vary): {pooled_sample_std:.3e}")
                print(f"        ACROSS samples (samples vary): {pooled_batch_std:.3e}")
                if pooled_batch_std < 1e-8:
                    print(f"        ⚠️  All samples have IDENTICAL embeddings!")
