...
class MemmapWindowDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        """Get a window with optional cropping (zero-copy from memmap) and target if available.

        Args:
            idx: Index in filtered metadata

        Returns:
            If 'target' in metadata: Tuple of (EEG tensor, target value)
            Otherwise: EEG tensor (n_channels, n_samples)
        """
        ...
        # Return (x, y) if target is available, otherwise just x
        if 'target' in self.metadata.columns:
            target = self.metadata.iloc[idx]['target']
            if pd.notna(target):
                return x_tensor, torch.tensor(float(target), dtype=torch.float32)
