"""S-JEPA fine-tuning with PostLocal architecture.
...
Applies spatial filtering AFTER the local encoder, operating on the learned
token representations rather than raw signals. This is a middle ground between
PreLocal (spatial filter first) and Contextual (full encoder + spatial filter).
"""
...
class PostLocalFinetuneTrainer(BaseSJEPAFinetuneTrainer):
    """Fine-tuning trainer using SignalJEPA_PostLocal architecture.

    Loads pretrained SignalJEPA base model and transfers weights to
    SignalJEPA_PostLocal variant via from_pretrained() method.
    ...
    def _load_model(self, checkpoint_path: str, n_outputs: int) -> nn.Module:
        """Load pretrained SignalJEPA and transfer to PostLocal variant.
        ...
        5. Call SignalJEPA_PostLocal.from_pretrained()
        ...
        Returns:
            SignalJEPA_PostLocal model with pretrained weights + new layers
        """
        checkpoint_path = Path(checkpoint_path)
        if not checkpoint_path.exists():
            raise FileNotFoundError(f"Checkpoint not found: {checkpoint_path}")

        print(f"[POSTLOCAL] Loading checkpoint from {checkpoint_path}")
        checkpoint = torch.load(checkpoint_path, map_location=self.device)

        # Extract encoder state dict
        encoder_state = self._extract_encoder_state(checkpoint)
        print(f"[POSTLOCAL] Extracted {len(encoder_state)} parameters from checkpoint")
        ...
        print("[POSTLOCAL] Loading pretrained weights into base SignalJEPA...")
        base_model.load_state_dict(encoder_state, strict=False)
        print("[POSTLOCAL] ✓ Loaded pretrained weights")

        # Transfer to PostLocal variant
        print(f"[POSTLOCAL] Creating PostLocal variant with n_spat_filters={self.n_spat_filters}...")
        model = SignalJEPA_PostLocal.from_pretrained(
            base_model,
            n_outputs=n_outputs,
            n_spat_filters=self.n_spat_filters
        )
        print("[POSTLOCAL] ✓ Created PostLocal model with pretrained weights")
