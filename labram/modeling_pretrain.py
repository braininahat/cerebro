...
class MEMPretrainModule(pl.LightningModule):
    ...
    LightningModule that trains MEM with labels from a VQ tokenizer:
      ...
      - tokenizer: any module exposing get_codebook_indices(x, input_chans)
    ...
    def __init__(
        self,
        model: NeuralTransformerForMEM,
        # your VQNSP tokenizer module (must have get_codebook_indices)
        tokenizer,
        patch_size: int = 200,
        lr: float = 1e-4,
        ...
        scale_input: bool = True,       # divide by 100 like engine
...
# -------------------------------
@register_model
def labram_base_patch200_1600_8k_vocab(pretrained=False, **kwargs):
...
@register_model
def labram_large_patch200_1600_8k_vocab(pretrained=False, **kwargs):
    kwargs.pop("num_classes", None)
    vocab_size = kwargs.pop("vocab_size", 8192)
    model = NeuralTransformerForMEM(
        EEG_size=1600,
...
@register_model
def labram_huge_patch200_1600_8k_vocab(pretrained=False, **kwargs):
    kwargs.pop("num_classes", None)
    vocab_size = kwargs.pop("vocab_size", 8192)
    model = NeuralTransformerForMEM(
        EEG_size=1600,
